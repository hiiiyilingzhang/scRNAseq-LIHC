---
title: "scRNAseq project-Tumor Cell Biodiversity Drives Microenvironmental Reprogramming in Liver Cancer"
author: "Yiling Zhang 18230130"
date: "`r Sys.Date()`"
output: 
...
vignette: >
  %\VignetteIndexEntry{scRNAseq project-Tumor Cell Biodiversity Drives Microenvironmental Reprogramming in Liver Cancer - Yiling Zhang 18230130}
...
knitr::include_graphics("../src/Figures/graphicab.jpg")
```

* **HCC（肝细胞癌）和iCCA（肝内胆管癌）具有不同程度的转录多样性;**
* **肿瘤转录多样性与患者预后相关;**
* **肿瘤来源的血管内皮生长因子驱动微环境重组;**
* **较高异质性来源的肿瘤T细胞具有较低的细胞溶解活性.**

---

```{r echo=FALSE,out.width = '75%',fig.align = "center"}
knitr::include_graphics("../src/Figures/Figure1.jpg")
```

* 为了研究肿瘤细胞的生物多样性，该研究用单细胞测序测了19个接受了免疫检查点治疗的肝细胞肝癌（HCC）和肝内胆管癌（iCCA）病人。其中12个病人组成一个训练集（5115个细胞），另外7个组成验证集（4831个细胞）；
* A为研究的流程，B图表明这12个病人(Set 1)的肿瘤细胞可以形成不同的cluster，有些独立，有些相互重叠。C图表明，有些细胞会有较高的表皮marker基因表达，提示这些肿瘤细胞可能有着相同的肿瘤起源；
* 通过inferCNVs，进一步区分了702个恶性细胞和4380个非恶性细胞(图1D)，并基于上皮标记基因和肝脏标记基因成功富集出恶性和非恶性细胞，这些标记基因的表达与恶性细胞和非恶性细胞的核型高度一致(图1E)。

---

```{r echo=FALSE,out.width = '75%',fig.align = "center"}
knitr::include_graphics("../src/Figures/Figure2.jpg")
```

* 通过对恶性和非恶性细胞进行t-SNE分析发现，恶性细胞形成了患者特异性细胞簇，同时也观察到肿瘤内部细胞同样存在异质性，表明肿瘤间和肿瘤内均存在转录组异质性(图2A);
* 非恶性细胞主要根据细胞类型进行分组(图2B)。文章比较了肿瘤患者中恶性细胞和非恶性细胞比例的变化(图2C)，并选择其中具有超过20个恶性细胞的8个样本进行分析。这8例患者中，基质细胞的组成同样具有高度异质性(图2D)。其中，H23、H30、H37、H38为HCC, C25、C26、C29、C39为iCCA(图2E)；

---

```{r echo=FALSE,out.width = '75%',fig.align = "center"}
knitr::include_graphics("../src/Figures/Figure3.jpg")
```

* 通过对10个肝癌干细胞相关基因EPCAM、KRT19、PROM1、ALDH1A1、CD24、ANPEP、CD44、ICAM1、CD47和SOX9的平均表达水平的比较，确定了肿瘤细胞的邻近相似性与细胞干细胞的关系(图3A)。并且通过对所有恶性细胞的相关性分析，进一步揭示瘤内转录组的多样性(图3B)；
* 为了进一步确定ITH的水平，文章开发了一种计算肿瘤细胞特异性转录组多样性得分的方法。使用PCs代替原始的恶性细胞基因表达谱来测量每个肿瘤的多样性(即ITH)(图3C)；
* 根据特征值排列选择前30个个体(图3D)。然后根据肿瘤样本的多样性得分将8个肿瘤样本分为多样性高(div -high)组和多样性低(div-low)组(图3E)；
* 多样性高组患者的总生存期和无进展生存期均明显低于多样性低组(图3F)。
* 对验证集（set2）应用多样性得分计算方法，分析显示，部分iCCA患者的多样性评分明显高于HCC患者(图3H)。

---

```{r echo=FALSE,out.width = '75%',fig.align = "center"}
knitr::include_graphics("../src/Figures/Figure4.jpg")
```
...
* 文章根据每种非恶性细胞类型的基因进行PCA分析。发现可以根据VEGFA区分div-high和div-low的TECs、TAMs和CAFs，但不能区分T细胞(图4I)。
...
### 3.1.1 Pre-process Seurat object (sctransform)  
```{r}
set1.obj <- SCTransform(set1.obj, verbose = F) %>% RunPCA(verbose = F)
```
...
```{r}
pc.num=1:20
set1.obj <- RunUMAP(set1.obj, dims=pc.num,verbose = F)
```

### 3.1.2 Optimize the parameters
...
```{r include=FALSE}
# saveRDS(set1.obj,"../data/tmp.data/rmdou.rds")
set1.obj <- readRDS("../data/tmp.data/rmdou.rds")
```
...
```{r echo=FALSE,out.width = '100%',fig.align = "center"}
knitr::include_graphics("../output/3.QC/QC-boxplot-gene.png")
```
...
**Visualizing PCA in FeaturePlot**
```{r}
...
**Visualizing PCA in ScatterPlot**
```{r}
...
```{r include=FALSE}
rm(hpca.se,p5,p6,pred.main.hpca,result_main_hpca,set1_forhpca.se,
   set1.test,common_hpca)
```
...
```{r echo=FALSE,out.width = '100%',fig.align = "center"}
knitr::include_graphics("../output/7.Annotation/tsne-compare-2.png")
```
...
```{r}
load("../data/inferCNV/cell-score.RData")
table(cell_scores_CNV)
```
...
```{r include=FALSE}
rm(cell_scores_CNV, cnv_score_table, cnv_table, cnv_score_mat)
...
Track_genes <- graph_test(my.cds, neighbor_graph="knn", cores=24, verbose = FALSE)
```

```{r include=FALSE}
# save(Track_genes, file = "../data/celltrajectory/Track-gene.RData")
load("../data/celltrajectory/Track-gene.RData")
```
...
```{r}
ann_colors = list( Clinical = c(HCC = "#D87F23", iCCA = "#A5A5A5"), Sample = c(H18 = "#fad3cf", H21 = "#2d095c", H23 = "#20366b", H28 ="#dd7777", H30 = "#eae3e3", H37 = "#2E94B9", H38 = "#FFFDC0", C25 = "#F0B775", C26 = "#D25565", C29 = "#a39e9e", C35 = "#2470a0", C39 = "#a696c8"))
```

```{r include=FALSE}
CairoPNG(file="../output/9.celltrajectory/ITH-heatmap.png",width=800,height=1000)
pheatmap::pheatmap(tmp.data, clustering_method="ward.D2",
...
```{r echo=FALSE,out.width = '100%',fig.align = "center"}
knitr::include_graphics("../output/9.celltrajectory/ITH-heatmap.png")
```

Compare with the result in the paper
```{r echo=FALSE,out.width = '60%',fig.align = "center"}
knitr::include_graphics("../src/Figures/corrplot.png")
```

---

```{r include=FALSE}
rm(agg_mat, cell_group, embed, gene_module, my.cds, p, Track_genes, genelist, root.cell, Track_genes_sig, annotation_col, annotation_row, ann_colors, tmp.data)
```
...
```{r identifyOverExpressGene/Interaction}
...
```{r}
my.cellchat <- readRDS("../data/cellchat/cellchat-obj.rds")
```
...
```{r include=FALSE}
CairoPNG(file="../output/10.Cellcellcommu/interactions.png",width=550,height=400)
groupSize <- as.numeric(table(my.cellchat@idents))
par(mfrow = c(1,2), xpd=TRUE)
netVisual_circle(my.cellchat@net$count, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Number of interactions")
netVisual_circle(my.cellchat@net$weight, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Interaction weights/strength")
...
CairoPNG(file="../output/10.Cellcellcommu/Number of interactions.png",width=480,height=480,)
netVisual_circle(my.cellchat@net$count, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Number of interactions")
dev.off()

CairoPNG(file="../output/10.Cellcellcommu/Interaction weights/strength.png",width=480,height=480,)
netVisual_circle(my.cellchat@net$weight, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Interaction weights/strength")
dev.off()

CairoPNG(file="../output/10.Cellcellcommu/FGF.png",width=480,height=480,)
netVisual_aggregate(my.cellchat, signaling = "FGF", layout = "chord")
dev.off()

CairoPNG(file="../output/10.Cellcellcommu/IL4.png",width=480,height=480,)
netVisual_aggregate(my.cellchat, signaling = "IL4", layout = "chord")
dev.off()

CairoPNG(file="../output/10.Cellcellcommu/FLT3.png",width=480,height=480,)
netVisual_aggregate(my.cellchat, signaling = "FLT3", layout = "chord")
dev.off()

CairoPNG(file="../output/10.Cellcellcommu/CD40.png",width=480,height=480,)
netVisual_aggregate(my.cellchat, signaling = "CD40", layout = "chord")
dev.off()
```
...
[GitHub-Velocyto](https://github.com/velocyto-team)  
[Homepage-Velocyto](http://velocyto.org/)  
...
If I don't use `job` package, R would be shut down easily! But RMarkdown doesn't support `job` package because it's actually a RStudio plugin.
```{r eval=FALSE}
job::job(kegg.info= { kegg <- gsva(expr, msigdbr_list, kcdf="Gaussian",method = "gsva", parallel.sz = 16)
})
save(kegg.info, file = "../data/Enrichment/kegg-info.RData")
```

```{r include=FALSE}
load("../data/Enrichment/kegg-info.RData")
```

```{r include=FALSE}
CairoPNG(file="../output/12.Enrichment/gsva_heatmap.png",width=1400,height=1800,)
pheatmap(kegg.info$kegg, show_rownames=1, show_colnames=0)
dev.off()
```


```{r}
es <- data.frame(t(kegg.info$kegg),stringsAsFactors=F)  
```

Add meta data
```{r}
set1.obj <- AddMetaData(set1.obj, es)
```

```{r fig.width=9}
p1 <- FeaturePlot(set1.obj, features = "KEGG_LIMONENE_AND_PINENE_DEGRADATION", reduction = 'umap')
...
Heatmap of every cluster and their function
```{r}
...
p <- pheatmap(result[1:20,],
                ...
                fontsize = 6)
pdf(("../output/12.Enrichment/gsva_celltype.pdf"),width = 8,height = 8)
print(p)
dev.off()
```

```{r include=FALSE}
rm(p,p1,p2,p3,p4,plotc,result,dat,group,i,ind,meta,msigdbr_list,m_df,kegg.info,expr,es,data1,data)
```

# Code and output availability
[Gitee-sc-rnaseq-lihc](https://gitee.com/yilingzhang/sc-rnaseq-lihc)
