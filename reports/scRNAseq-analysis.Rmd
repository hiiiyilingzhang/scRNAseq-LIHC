...
CairoPNG(file="../output/2.2Examine Data/CountsPerCell.png",width=480,height=410,)
hist(log10(counts_per_cell+1),main='counts per cell',col='Thistle')
dev.off()
```
...
```{r echo=FALSE,eval=TRUE}
CairoPNG(file="../output/2.2 Examine Data/GenesPerCell.png",width=480,height=410,)
hist(log10(genes_per_cell+1), main='genes per cell', col='Thistle')
dev.off()
```
...
```{r echo=FALSE,eval=TRUE}
CairoPNG(file="../output/2.2 Examine Data/Counts&GenesPerCell.png",width=480,height=410,)
plot(counts_per_cell, genes_per_cell, log='xy', col='Thistle')
title('counts vs genes per cell')
dev.off()
```
...
```
```{r echo=FALSE,eval=TRUE}
CairoPNG(file="../output/2.2 Examine Data/GenesPerCell(ordered).png",width=480,height=410,)
plot(sort(genes_per_cell), xlab='cell', log='y', main='genes per cell (ordered)')
dev.off()
...
```{r}
load("../output/3.1 Remove Doublets/pk_bcmvn.RData")
```
...
```{r}
## count
table(set1.obj$DF.classifications_0.25_0.03_98)
```
...
```{r}
rm(selected_c,selected_f, most_expressed, C)
```
...
```{r warning=FALSE}
# run sctransform 
set1.obj <- SCTransform(set1.obj, vars.to.regress = "percent.mt", verbose = FALSE, 
                        seed.use = 2021) 
set1.obj
```
...
FeaturePlot(object = set1.obj, features = "MS4A1")
```
3. Visualizing PCA in ScatterPlot
```{r}
# Scatter plot across single cells, replaces GenePlot
FeatureScatter(object = set1.obj, feature1 = "MS4A1", feature2 = "PC_1")
```
...
```{r}
# Heatmaps
DimHeatmap(object = set1.obj, reduction = "pca", cells = 200, balanced = TRUE)
```
...
```{r}
set1.obj <- FindNeighbors(set1.obj, reduction = "pca", dims = 1:30)
```
...
# every cell name
DoHeatmap(object = set1.obj, features = top10$gene, label = TRUE,group.by = "Clinical.Info")
```
...
```{r}
rm(top10, cluster1.markers, markers)
```
...
set1.obj@meta.data <- merge(set1.obj@meta.data,result_main_hpca,by="CB")
...
plot_grid(p6, p5,rel_widths = c(2,3))
```

```{r echo=FALSE,eval=TRUE}
CairoPNG(file="../output/7. Annotation/Compare.png",width=1080,height=400,)
plot_grid(p6, p5,rel_widths = c(2,3))
dev.off()
```
...
```{r echo=FALSE,out.width = '60%',fig.align = "center"}
knitr::include_graphics("../output/8.inferCNA/results/infercnv.png")
```

```{r}
rm(cellAnno,dat, geneInfor, infercnv_obj,rawexpr)
```

## 8.3 Further annotation
```{r echo=FALSE,out.width = '100%',fig.align = "center"}
knitr::include_graphics("../src/Figures/non-malignant.png")
```

```{r}
celltype_marker <- c(
  "CD79A","SLAMF7","BLNK","FCRL5", #B cell
  "CD3E","CD3D","CD3G", "CD2", #T cell
  "PECAM1","VWF","ENG","CDH5", #TECs
  "COL1A2","FAP","PDPN","DCN","COL3A1","COL6A1", #CAFs
  "CD14", "CD163", "CD68", "CSF1R", #TAMs
  "EPCAM", "KRT19", "PROM1", "ALDH1A1", "CD24" #HPClike
)
DotPlot(set1.obj, features = celltype_marker, group.by = "Clinical.Info") + coord_flip()
```
```{r}
library(ggplot2)
p1 <- DimPlot(object = set1.obj, reduction = "tsne",group.by = "Type")
p1<- ggplot(p1$data, aes(tSNE_1, tSNE_2,fill = Type,col = Type))+
  geom_point(aes(color=Type))+
  stat_ellipse(level = 0.9,alpha = 1/5,geom = "polygon")+
  theme_bw()+
...
```{r include=FALSE}
rm(p1, celltype_marker)
saveRDS(set1.obj, file = "../data/tmp.data/set1-anno.rds")
```
...
expression_matrix <- set1.obj@assays$RNA@counts
expression_matrix_df <- set1.obj@assays$RNA@counts %>% dgc_as_matrix() %>% as.data.frame()
...
```{r}
# Get cell metadata
cell_metadata <- set1.obj@meta.data
head(cell_metadata)
```
```{r}
cell_metadata_sort <- data.frame(matrix(NA, 4322,2))
...
gene_annotation <- data.frame(gene_short_name = rownames(set1.obj@assays$RNA), row.names = rownames(set1.obj@assays$RNA))
...
# umap降维
my.cds <- reduce_dimension(my.cds, preprocess_method = "UMAP")
...
```{r}
##从seurat导入整合过的umap坐标
my.cds@int_colData@listData$reducedDims$UMAP <- set1.obj@reductions$umap@cell.embeddings
...
my.cds@clusters$UMAP_so$clusters <- set1.obj@meta.data$DF.classifications_0.25_0.03_98
```
...
```{r}
plot_cells(my.cds, color_cells_by = "partition", show_trajectory_graph = FALSE) + ggtitle("label by partitionID")
```
...
p <- plot_cells(my.cds, label_groups_by_cluster = FALSE, label_leaves = FALSE, 
...
# saveRDS(my.cds,"../output/9. Monocle3/mycds.rds")
my.cds<-readRDS("../output/9. Monocle3/mycds.rds")
